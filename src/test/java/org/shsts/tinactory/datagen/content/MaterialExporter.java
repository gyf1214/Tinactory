...
public final class MaterialExporter {
    ...
    private static void exportMaterial() throws IOException {
        Files.createDirectories(Paths.get("materials"));
        ...
        for (var mat : AllMaterials.SET.values()) {
            ...
            for (var sub : mat.fluidSubs()) {
                var target = mat.fluid(sub).get() == Fluids.WATER ? "liquid" :
                    LocHelper.name(mat.fluidLoc(sub).getPath(), -2);
